	...
	def _init_broker(self, quiet=True, timeout=5):
		try:
			with open(LockFilePath, "a") as lockFile:
				...
				self._delete_silently("/data/conf/mosquitto.d/vrm_bridge.conf");
				self._delete_silently("/run/mosquitto/vrm_bridge.conf");
				...
				try:
					...
					settings = dict(tuple(l.strip().split(' ', 1)) for l in orig_config.split('\n')
						...
						and "{" not in l.strip() and "}" not in l.strip())
					self._global_broker_password = settings.get('password')
				except IOError:
					if not quiet:
